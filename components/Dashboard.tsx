...
export default function Dashboard() {
  ...
  const weatherApiKey = locationData
    ? `/api/weather?lat=${locationData.latitude}&lon=${locationData.longitude}`
    : null;
  ...
  // 3. Fetch Quote Data
  const { data: quoteData, error: quoteError } = useSWR('/api/quote', fetcher, {
    revalidateOnFocus: false,
    revalidateOnReconnect: true,
    errorRetryCount: 3,
    errorRetryInterval: 5000,
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            We couldn&apos;t determine your location. This might be due to
            network restrictions or privacy settings.
          </motion.p>
          <motion.button
    ...
    <ErrorBoundary>
      ...
      <div className='w-full max-w-6xl mx-auto px-4 py-8 animate-fade-in'>
        ...
        <div className='text-center mb-12 animate-slide-up'>
          <div className='flex items-center justify-center gap-4 mb-4'>
            <h1 className='text-5xl md:text-6xl font-bold gradient-text leading-tight'>
              {greeting}, {userName || locationData?.city || 'User'}!
            </h1>
            {userName && (
              <motion.button
                whileHover={{ scale: 1.05 }}
                ...
                className='text-aurora-green hover:text-aurora-blue transition-colors duration-200 text-sm opacity-60 hover:opacity-100'
                title='Change name'
              >
                ✏️
              </motion.button>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8'>
          ...
          >
            <QuoteOfTheDay data={quoteData} error={quoteError} />
          </div>
        </div>
        ...
        {hasErrors && (
          <div className='mt-8 p-4 bg-red-500/10 border border-red-500/20 rounded-lg animate-bounce-in'>
            ...
            <p className='text-gray-400 text-sm'>
              Don&apos;t worry, you can still enjoy your dashboard. Try
        ...
        <footer className='text-center mt-12 text-gray-500 text-sm'>
          ...
          <p className='mt-1'>
            Data provided by WeatherAPI.com and Quotable.io
          </p>
        </footer>
      </div>
    </ErrorBoundary>
